...
class TestLargestSum:
    ...
    @pytest.mark.parametrize("given_input,expected", [
        ([1, 2, 3], 4), ([3, 5, 1], 5), ([-10, 15, 20], 20)
    ])
    def test_list_of_3_ints_should_return_max_non_adjacent_sum(self, given_input, expected):
        assert largest_non_adjacent_sum(given_input) == expected

    @pytest.mark.parametrize("given_input,expected", [
        ([1, 2, 3, 4], 6), ([3, 1, 2, 4], 7), ([3, 4, -1, -3], 4)
    ])
    def test_list_of_4_ints_should_return_max_non_adjacent_sum(self, given_input, expected):
        assert largest_non_adjacent_sum(given_input) == expected
