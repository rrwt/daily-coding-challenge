...
def split_arr(arr: list, arr_len: int, ind: int, arr1: list, arr2: list) -> Tuple[bool, list, list]:
    """
    ...
    for index in range(1, arr_len + 1):
        ...
        for cur_total in range(1, total+1):
            if cur_total >= arr[index-1]:
                dp[index][cur_total] = dp[index-1][cur_total] or dp[index-1][cur_total-arr[index-1]]
            else:
